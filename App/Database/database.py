...
from ..Config.config import DB_NAME, DB_HOST, DB_PASS, DB_USER
...
class DB:
    ...
    def update(self, table: str, columns: list, where: str = None):
        # Example: update('users', ['lang'], 'userid = ?', [userid])
        # Example multiple where: update('users', ['lang'], f"userid = {userid} AND lang = {lang}")        

        self.cursor.execute(f'UPDATE {table} SET {",".join([f"{column} = ?" for column in columns])}' + (f' WHERE {where}' if where else ''))
        self.conn.commit()
        self.send_backup()
